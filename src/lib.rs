...
pub fn encode_config(bytes: &[u8], config: Config) -> String {
    ...
    encode_config_buf(bytes, config, &mut buf);
...
///}
///```
pub fn encode_config_buf(bytes: &[u8], config: Config, buf: &mut String) {
    let charset = match config.char_set {
        CharacterSet::Standard => STANDARD_CHARSET,
        CharacterSet::UrlSafe => URL_SAFE_CHARSET,
    };
...
pub fn decode_config_buf(input: &str, config: Config, buffer: &mut Vec<u8>) -> Result<(), Base64Error> {
    let decode_table = match config.char_set {
        CharacterSet::Standard => decode_tables::STANDARD,
        CharacterSet::UrlSafe => decode_tables::URL_SAFE,
    };
